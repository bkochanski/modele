...
\begin{equation}
\sigma_{XY}=E[\left(X-E(X)\right)(Y-E(Y))]
(\#eq:covar-intext)
\end{equation}
...
\begin{equation}
\sigma_{XY}=\sum_i\left(x_i-E(X)\right)(y_i-E(Y))P(X=x_i \land Y=y_i)
...
Korelacja przyjmuje wartości od -1 do 1, może więc służyć do porównywania siły współzależności pomiędzy różnymi zmiennymi. 
...
Jaka jest wartość oczekiwana, wariancja i odchylenie standardowe wypłaty w tej grze, jeżeli gracz dysponuję stawką 1 USD i 
