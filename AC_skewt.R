...
if (from > to) {
  ...
  result<-sn::pst(to, m, s, a, g)-sn::pst(from, m, s, a, g)
  print(result)
}
...
plt<-ggplot(NULL, aes(c(x1, x2))) +
  ...
  geom_area(stat = "function", 
            fun = function(x){sn::dst(x, m, s, a, g)}, 
            fill = "orange", 
            xlim = c(if(from==-Inf){x1}else{from}, if(to==Inf){x2}else{to})) +
  geom_line(stat = "function", fun = function(x){sn::dst(x, m, s, a, g)}, col = "blue", lty=2, lwd=1) +
  ...
  annotate("text", label = paste0("m = ", m, "\ns = ", s, "\na = ", a, "\ng = ", g, "\n", p, " = ", signif(result,6)), 
           x = x1, y = sn::dst(m, m, s, a, g)*1.2, size = 6, hjust="inward", vjust = "inward")
