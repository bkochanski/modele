...
\begin{equation}  
\bar{\mathcal{R}} = \frac{\sum_{i=1}^n \mathcal{R}_i}{n} = 1 + \bar{R}
(\#eq:sredniaRp-intext)
\end{equation}
...
Stopy netto mogą być ujemne, więc generalna definicja nie ma zastosowania. W matematyce finansowej i w praktyce średnią geometryczną stopą zwrotu (netto) nazywa się niekiedy geometryczną średnią ze stóp zwrotu brutto w następujących po sobie okresach pomniejszoną o 1 [@DeFuscoEtAl_2007]:

\begin{equation}  
R_G = \left(\prod_{i=1}^t \mathcal{R}_i\right)^{\frac{1}{t}} - 1 = \left[\prod_{i=1}^t (1+R_i)\right]^{\frac{1}{t}} - 1
(\#eq:sredniagR-intext)
\end{equation}
